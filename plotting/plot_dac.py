...
def main(fname):
    ...
    save_dir = os.path.join(os.path.dirname(fname), 'dac_plots')
    os.makedirs(save_dir, exist_ok=True)
    deploy = fname.split('/')[-1].split('_')[0]

    t0str = pd.to_datetime(np.nanmin(ds.UnixTime)).strftime('%Y-%m-%dT%H:%M')
    t1str = pd.to_datetime(np.nanmax(ds.UnixTime)).strftime('%Y-%m-%dT%H:%M')

    plt_vars = cf.plot_vars_dac()
    for pv, info in plt_vars.items():
        ...
        if len(variable) > 1:
            ...
            xargs['date_fmt'] = '%m-%d'
            xargs['grid'] = True
            xargs['ylabel'] = ds.Pressure.units
            xargs['cmap'] = info['cmap']
            pf.xsection(fig, ax, ds.UnixTime.values, ds.Pressure.values, variable.values, **xargs)
